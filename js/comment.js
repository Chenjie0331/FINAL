...
const templateProductDiv = container.querySelector(".product-div1");
container.innerHTML = ""; // 清空原本的範本 不然的話會多出一個標籤欄位
 
fetch("json/comment.json", {
  method: "GET",
})
  .then(async (res) => {
    ...
    response.forEach((item) => {
      ...
      productDiv.querySelector(".name").textContent = item.name;
      productDiv.querySelector(".image").src = item.image;
      productDiv.querySelector(".discription").textContent = item.discription;
